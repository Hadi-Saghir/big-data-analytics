...
class CloneDetector {
        ...
        let newInstances = file.chunks.flatMap(chunk => {
            ...
            return matchingChunks.map(matchingChunk => {
                try {
                    ...
                    // Assuming file.name and compareFile.name are the appropriate source and target names.
                } catch (err) {
                    console.error(err.message);
                    return null;
                }
            }).filter(clone => clone !== null);  // Filter out any null values
        });
    ...
    #expandCloneCandidates(file) {
        ...
        // (using Clone::maybeExpandWith(), which returns true if it could expand) - Somebody be doin some c++
        // 
        ...
        file.instances = file.instances.reduce((accumulator, currentClone) => {
            // Check if currentClone overlaps with any Clone in the accumulator
            let overlappingClone = accumulator.find(accClone => accClone.maybeExpandWith(currentClone));

            // If no overlap, add currentClone to the accumulator
            if (!overlappingClone) {
    ...
    #consolidateClones(file) {
        ...
        // reduce
        file.instances = file.instances.reduce((accumulator, currentClone) => {
            // Check if currentClone is equivalent to any Clone in the accumulator
            let existingClone = accumulator.find(accClone => accClone.equals(currentClone));

            // If there's an equivalent Clone, update it using addTarget
            if (existingClone) {
                existingClone.addTarget(currentClone);
            } else {
                // If there's no equivalent Clone, add currentClone to the accumulator
                accumulator.push(currentClone);
            }
