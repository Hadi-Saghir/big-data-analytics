...
app.get('/timing-stats', (req, res) => {
    ...
    page += '<table border="1">\n';
    page += '<tr><th>File Name</th><th>Processing Time (ms)</th><th>Line Count</th><th>Normalized Time</th></tr>\n';
    for (let stat of timingStats) {
        page += `<tr><td>${stat.fileName}</td><td>${stat.processingTime}</td><td>${stat.lineCount}</td><td>${stat.normalizedTime.toFixed(2)}</td></tr>\n`;
    }
    page += '</table>\n';
    page += '</BODY></HTML>';
    res.send(page);
    ...
    return Promise.resolve({name: filename, contents: contents} )
        ...
        .then( file => {
            ...
            timingStats.push({
                ...
                normalizedTime: processingTime / contents.split('\n').length
            });
